...
def train(args):
    ...
    print("check if your code is running on GPU:", device)
    #### Load data
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE)):
            ...
            ### TODO: 
            ### 1. reset the graidnet after iteration
            ### 2. backprop the loss
...
if __name__ == "__main__":
    args = get_args()
    dataset = os.path.basename(args.train).split('-')[0]
    args.filepath = f'{dataset}-{args.option}-{args.epochs}-{args.lr}.pt'  # save path
